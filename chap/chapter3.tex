...
Dies geschieht dadurch das in der Mitte der Platte ein kleines Loch vorhanden ist durch welches der Elektronenstrahl nun fließt. 
...
Des Weiteren wird die Spannung mit 1000 multipliziert, da in der Formel mit V gerechnet wird, während die Spannung der Elektronenkanone in kV angegeben ist. Deswegen muss diese noch umgerechnet werden.
...
Dies bedeutet, dass Sie deswegen als Zentripetalkraft auf die Elektronen wird und diese dadurch auf eine Kreisbahn zwingt.
...
Im Folgenden ist der Code in der Simulation dargestellt.
Hier wird zuerst ebenfalls der \lstinline$Bahnradius$ berechnet, dies geschieht dadurch, dass die Elektronenmasse und die zuvor berechnete Teilchengeschwindigkeit mit aus dem Strahl geholt werden und miteinander multipliziert werden.
Des Weiteren wird dieses Ergebnis durch die Elektronenladung und der Magnetfeldstärke mal 1000 dividiert. Die Elektronenladung wird sich ein weiteres mal aus der Klasse Strahl geholt, wo sie gespeichert wurde.
...
Die Situation ist wie in der Skizze in der Abbildung \ref{fig:ausBlog}.\footnote{Diese Abbildung wurde übernommen aus \url{https://www.physikerboard.de/ptopic,342494.html#342494}.}
...
Bei der Berechnung des Winkels $\alpha$ wird die oben bereits genannte Formel \ref{eq:tan} mit zwei multipliziert da auf der Ausgabe der tatsächliche Winkel $\alpha$ zu sehen sein soll.
Des Winkel $\alpha$ wird im Bogenmaß berechnet.
Bei der \lstinline$Ablenkungsrichtung$ wird der Richtungsvektor des Strahl mit $-1$ multipliziert und danach das Kreuzprodukt mit dem Richtungsvektor des Ablenkspulenpaars genommen.
...
Durch den for Befehl wird bewirkt, dass wie oben bereits genannt die Berechnung des \lstinline$Ergebnisvektors$ für jeden der Magneten gemacht werden muss.
Dies bedeutet, dass der \lstinline$Ergebnisvektor$, welcher zu Beginn auf\lstinline$bildschirmabstand$ als x- Koordinate, $0$ als y-Koordinate und 0 als z-Koordinate gesetzt ist, erst die Formel \ref{eq:v} benutzt wird, dieses Ergebnis wird dann als \lstinline$Ergebnisvektor$ gespeichert.
Danach wird die Formel \ref{eq:v} erneut benutzt, allerdings ist der \lstinline$Ergebnisvektor$ nun der Vektor, welcher im Schritt davor als Ergebnis rausgekommen ist.
